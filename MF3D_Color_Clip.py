...
##                clip_slot.set_triggered_to_launch_value(1)
    ...
    def _set_colors(self, scene_dir):
        ...
        for scene_index in range(4):
            ...
            self.log_message("scene index :" + str(scene_index) + " scene offset " + str(scene_off))
            ...
            for track_index in range(4):
                ...
                if c.clip != None:
                    cval=self._get_MF3D_color(self.int_to_rgb(c.clip.color))
                    clip_slot.set_stopped_value(cval)
                    self.log_message("Clip:  scene: " + str(scene_index+scene_off) + " track: " + str(track_index) + " has color: " + str(self.int_to_rgb(c.clip.color)) + " va " + str(cval))
    ...
    def update_display(self):
        ...
        if (scene_off!=self._last_sc_off):
            self.log_message("moved : "+ str(self._session._track_offset)+ " " +str(self._session._scene_offset))
            if (scene_off>self._last_sc_off):
